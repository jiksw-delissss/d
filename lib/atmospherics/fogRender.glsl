...
float getFogFactor(in float viewDist, in float nEyePlayerPosY, in float worldPosY){
        ...
        float totalFogDensity = isEyeInWater == 0 ? FOG_TOTAL_DENSITY : FOG_TOTAL_DENSITY * TAU * UNDERWATER_FOG_MULT;
    #else
        ...
        float totalFogDensity = isEyeInWater == 0 ? FOG_TOTAL_DENSITY * (rainStrength * eyeBrightFact * PI + 1.0) : FOG_TOTAL_DENSITY * TAU * UNDERWATER_FOG_MULT;
    #endif
