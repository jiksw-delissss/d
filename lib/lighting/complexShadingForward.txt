vec4 complexShadingForward(in dataPBR material){
    ...
    float skyLightSquared = squared(lmCoord.y);
        ...
        // Provide a safe fallback for `entityId` when compiling shaders that don't
    // declare the uniform. Some shaders (terrain/solid) don't provide the
    // `uniform int entityId;` declaration; if so, declare a harmless constant
    // fallback. Files that do declare the uniform will define `HAS_ENTITY_ID`
    // to prevent redeclaration.
    #ifndef HAS_ENTITY_ID
    ...
    vec3 skyIllum = (toLinear(SKY_COLOR_DATA_BLOCK) + lightningFlash) * skyLightSquared;

    // Reduce the sky contribution when rendering entities to avoid strong
    // unwanted skybox reflections on shiny entity surfaces. Apply the tweak
    // when either the `ENTITIES` build flag or the local `HAS_ENTITY_ID`
    // marker (set in entity GBuffers) is present so all entity passes are
    // covered.
    #if defined(ENTITIES) || defined(HAS_ENTITY_ID)
        skyIllum *= 0.25;
    #endif
        ...
        bool isSubSurface = material.ss > 0;
            ...
            if(isShadow || isSubSurface){
                ...
                // Sample shadows
                    ...
                    shdCol = getShdCol(shdPos, dither);
                #else
    ...
    vec4 totalLighting = vec4(material.albedo.rgb * totalIllumination, material.albedo.a);
        ...
        if(isShadow){
            ...
            // Boost moon specular to match sun brightness — moon spreads on water same as sun
            vec3 specLightCol = mix(sRGBLightCol * 5.0, sRGBLightCol, DAY_BRIGHTNESS);
            totalLighting.rgb += sunMoonIntensitySqrd * specCol * specLightCol;
            if(material.albedo.a != 1) totalLighting.a = min(maxOf(specCol) + totalLighting.a, 1.0);

            // Sun specular highlight for terrain and entities: simple Blinn-Phong
            // added on top of diffuse without touching fresnel/reflections.
            // Only fires for blocks/entities with default smoothness (0.0).
            if(material.smoothness < 0.01){
                vec3 sunDir = vec3(shadowModelView[0].z, shadowModelView[1].z, shadowModelView[2].z);
                vec3 H = fastNormalize(sunDir + viewDir);
                float NdotH = max(dot(material.normal, H), 0.0);
                float exponent = 1.0 / max(1.0 - SUN_SPECULAR_MIN_SMOOTHNESS, 0.001);
                #if defined SHADOW_MAPPING
                    float shdScalar = maxOf(shdCol);
                #else
                    float shdScalar = shdCol;
                #endif
                float highlight = pow(NdotH, exponent * 8.0) * NLZ * shdScalar;
                totalLighting.rgb += highlight * TERRAIN_SUN_SPECULAR_INTENSITY * sRGBLightCol * sunMoonIntensitySqrd;
            }
        }
    #endif
