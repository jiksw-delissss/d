...
/// -------------------------------- /// Vertex Shader /// -------------------------------- ///
    ...
    // Pass shadow position from vertex shader to fix precision issues
    ...
    void main(){
        ...
        // Calculate shadow position in vertex shader for precision
        // MUST be done after WORLD_CURVATURE to match dh_shadow.glsl
        #if defined WORLD_LIGHT && defined SHADOW_MAPPING
            vec3 shadowPosTransformed = mat3(shadowModelView) * vertexFeetPlayerPos + shadowModelView[3].xyz;
            shadowPosTransformed = vec3(shadowProjection[0].x, shadowProjection[1].y, shadowProjection[2].z) * shadowPosTransformed + shadowProjection[3].xyz;

            // Apply shadow distortion (matches dh_shadow.glsl)
            shadowPosTransformed.xy = shadowPosTransformed.xy / (length(shadowPosTransformed.xy) + 0.1);
            shadowPosTransformed.z *= 0.2;

            // Normalize to 0.0 - 1.0 range
            shadowPos = shadowPosTransformed * 0.5 + 0.5;
        #endif
