...
/// -------------------------------- /// Vertex Shader /// -------------------------------- ///
        ...
        void main(){
            ...
            #if defined(RAYTRACING_DEBUG_VIEW) || defined(VOXEL_RT_REFLECTIONS) || defined(VOXEL_RT_LIGHTING)
                // Same as shadow_cutout.glsl: full-face fill first, corner-write fallback
                // for non-blocky shapes (torches, flowers, crops, etc.) so they render in
                // RT reflections too, just as a rougher single-cell proxy.
                if(!sdv_writeVoxelFace(vertexShdViewPos, texCoord, vertexColor)){
                    sdv_writeVoxel(vertexShdViewPos, texCoord, vertexColor);
                }
            #endif
...
/* RENDERTARGETS: 0,1 */
        ...
        void main(){
                ...
                if(shdAlbedo.a == 1){
                    shadowColOut    = vec3(0);
                    shadowAlbedoOut = shdAlbedo.rgb * vertexColor; // opaque albedo for GI
                    return;
                }

                if(blockId != 11102){
                    shadowColOut    = shdAlbedo.rgb;
                    shadowAlbedoOut = shdAlbedo.rgb * vertexColor;
                    return;
                }
                ...
                shadowColOut    = vec3(0);
                shadowAlbedoOut = textureLod(gtexture, texCoord, 0).rgb;
            #endif
