...
/// -------------------------------- /// Vertex Shader /// -------------------------------- ///
        ...
        void main(){
            ...
            #if defined(RAYTRACING_DEBUG_VIEW) || defined(VOXEL_RT_REFLECTIONS) || defined(VOXEL_RT_LIGHTING)
                // RT voxel reflections are intentionally block-only.
                // Entities and all non-blocky shapes are skipped so they do not
                // get collapsed into the coarse RT proxy and cause voxel artifacts.
                // No sdv_writeVoxel call here.
            #endif
...
/// -------------------------------- /// Fragment Shader /// -------------------------------- ///
    ...
    #ifdef WORLD_LIGHT
        /* RENDERTARGETS: 0,1 */
        layout(location = 0) out vec3 shadowColOut;
        layout(location = 1) out vec3 shadowAlbedoOut;
