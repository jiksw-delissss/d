...
/// -------------------------------- /// Fragment Shader /// -------------------------------- ///
        ...
        void main(){
                ...
                // Use water derivatives to create a simple caustic focusing effect
                // when underwater caustics are enabled. Falls back to cell-noise
                // if the H2NWater helper is not available.
                #ifdef WATER_FLAT
                    shadowColOut = vec3(0.8);

                    #if UNDERWATER_CAUSTICS != 0
                        vec3 sunVec = vec3(0.0, 1.0, 0.0);
                        vec3 wPos = vec3(waterNoiseUv.x / waterTileSizeInv, 0.0, waterNoiseUv.y / waterTileSizeInv);
                        float caust = saturate(waterCaustics(wPos, sunVec, 2.0) * WATER_CAUSTIC_BRIGHTNESS);
                        vec3 causticColor = mix(vec3(0.2), vec3(0.3, 0.85, 1.0) * (0.9 * caust + 0.1), caust);
                        #if UNDERWATER_CAUSTICS == 2
                            shadowColOut = causticColor;
                ...
                shadowColOut = toLinear(shadowColOut);
            #else
                // Alpha test, discard and return immediately
